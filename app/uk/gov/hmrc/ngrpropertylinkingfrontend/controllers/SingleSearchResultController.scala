...
class SingleSearchResultController @Inject(singleSearchResultView: SingleSearchResultView,
  ...
  def show(page: Int = 1, sortBy: String = "AddressASC"): Action[AnyContent] =
    (authenticate andThen isRegisteredCheck).async { implicit request =>
      findAPropertyRepo.findByCredId(CredId(request.credId.getOrElse(""))).flatMap{
        case Some(properties) =>
          showSingleSearchResultView(properties, page, sortBy)
  ...
  def sort: Action[AnyContent] =
    (authenticate andThen isRegisteredCheck).async { implicit request =>
      println(Console.RED + "===============1" + Console.RESET)
      form
        .bindFromRequest()
        .fold(
          formWithErrors =>
          ...
          ,
          singleSearchResult => {
            println(Console.RED + "===============2" + Console.RESET)
            findAPropertyRepo.findByCredId(CredId(request.credId.getOrElse(""))).flatMap {
