...
import uk.gov.hmrc.ngrpropertylinkingfrontend.models.{Link, NGRSummaryListRow, UploadViewModel, UpscanInitiateResponse, UpscanRecord}
        ...
        upscanRepo.findByCredId(credId).flatMap {
          case Some(record) =>
            ...
            propertyLinkingRepo.findByCredId(CredId(request.credId.getOrElse(""))).flatMap {
              case Some(property)
              => Future.successful(Ok(uploadedView(createDefaultNavBar, createSummaryList(fileName, record.status, record.downloadUrl), property.vmvProperty.addressFull)))
